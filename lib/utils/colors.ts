/**
 * Tailwind color utilities for mapping color names to hex values
 */

// Map of Tailwind colors to their hex values
 ...
 *
 * Examples:
 * - "#ff0000" -> "#ff0000" (direct pass-through)
...
export function resolveColor(color?: string): string {
  ...
  // Check if it's a Tailwind color pattern (color-shade)
  const match = color.match(/^([a-z]+)-([0-9]+)$/);
  if (match) {
    const [, colorName, shade] = match;
    const shadeNum = Number.parseInt(shade, 10);

    // Check if the color exists in our map
    if (tailwindColors[colorName]?.[shadeNum]) {
      return tailwindColors[colorName][shadeNum];
    }
  }
